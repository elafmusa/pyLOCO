...
delta_correction = skew_deltas  ##### for skew also #skew_deltas
#delta_correction = np.atleast_1d(delta_correction).astype(float)

if len(skew_deltas) != len(quads_names):
    raise ValueError(
    f"JSON contains {len(skew_deltas)} corrections "
