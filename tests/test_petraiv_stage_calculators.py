...
def test_two_stage_wrapper_passes_independent_orm_calculators(driver, monkeypatch, tmp_path):
    ...
    assert all(call["analytical_formula_use_mp"] is False for call in calls)
    assert all(call["analytical_dispersion_worker"] == "rf_only" for call in calls)
    assert all(call["analytical_dispersion_difference"] == "central" for call in calls)
    assert all(call["analytical_dispersion_step_metric"] == "full_orm" for call in calls)
    assert all(call["analytical_dispersion_worker_transport"] == "initializer" for call in calls)
    assert all(call["analytical_dispersion_worker_chunksize"] == 1 for call in calls)
    assert all(call["analytical_dispersion_reuse_adaptive_plus_rf"] is True for call in calls)
    assert all(call["skew_analytical_dispersion_calculator"] == "Linear" for call in calls)
    assert all(call["skew_analytical_formula_use_mp"] is False for call in calls)
    assert all(call["skew_analytical_dispersion_worker"] == "rf_only" for call in calls)
    assert all(call["skew_analytical_dispersion_difference"] == "central" for call in calls)
    assert all(call["skew_analytical_dispersion_step_metric"] == "full_orm" for call in calls)
    assert all(call["skew_analytical_dispersion_worker_transport"] == "per_task" for call in calls)
