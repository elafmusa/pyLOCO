...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._machine_label = QLabel("Machine: Not selected")
        self._orm_label = QLabel("ORM: Not loaded")
        self._dispersion_label = QLabel("Dispersion: Not loaded")
        self._mode_label = QLabel("Mode: Basic")

        self._workspace = self._create_workspace()
        self._project_explorer = ProjectExplorer()
        self._project_explorer.setMinimumWidth(280)
        ...
        self._project_explorer.set_mode("Basic")
    ...
    def _create_workspace(self) -> QTabWidget:
        ...
        dashboard.new_project_requested.connect(lambda: self._show_placeholder("New Project"))
        dashboard.open_project_requested.connect(lambda: self._show_placeholder("Open Project"))
        dashboard.documentation_requested.connect(lambda: self._show_placeholder("Documentation"))
        dashboard.examples_requested.connect(lambda: self._show_placeholder("Examples"))
        tabs.addTab(dashboard, "Project")
    ...
    def _create_actions(self) -> None:
        ...
        self.new_project_action = QAction(
            self.style().standardIcon(QStyle.SP_FileIcon), "New Project", self
        )
        self.new_project_action.setShortcut(QKeySequence.New)
        self.new_project_action.setToolTip("Create a new pyLOCO project")
        ...
        self.open_project_action = QAction(
            self.style().standardIcon(QStyle.SP_DirOpenIcon), "Open Project…", self
        )
        self.open_project_action.setShortcut(QKeySequence.Open)
        self.open_project_action.setToolTip("Open an existing pyLOCO project")
        ...
        self.save_project_action.setToolTip("Save the current project")
        ...
        self.validate_project_action = QAction(
            self.style().standardIcon(QStyle.SP_DialogApplyButton), "Validate Project", self
        )
        self.validate_project_action.setToolTip("Validate project inputs")
        ...
        self.basic_mode_action = QAction(
            self.style().standardIcon(QStyle.SP_DialogYesButton), "Basic", self
        )
        self.basic_mode_action.setCheckable(True)
        self.basic_mode_action.setChecked(True)
        self.basic_mode_action.setToolTip("Use guided workflow pages and presets")
        self.basic_mode_action.setStatusTip("Use guided workflow pages and presets")

        self.advanced_mode_action = QAction(
            self.style().standardIcon(QStyle.SP_ComputerIcon), "Advanced", self
        )
        self.advanced_mode_action.setCheckable(True)
        self.advanced_mode_action.setToolTip("Show the complete advanced project tree")
    ...
    def _create_menu_bar(self) -> None:
        ...
        mode_menu.setIcon(self.style().standardIcon(QStyle.SP_FileDialogContentsView))
    ...
    def _create_status_bar(self) -> None:
        """Build the status bar with useful project state placeholders."""
    ...
    def _apply_polished_style(self) -> None:
        """Apply shell-level spacing, font, and color refinements."""
        ...
        self.setStyleSheet(
            ...
            QTabBar::tab:selected {
                background: #ffffff;
                border-bottom: 3px solid #2f6fbb;
                font-weight: 600;
            }
            QDockWidget {
            ...
            #dashboardSubtitle, #cardDescription, #mutedText {
                color: #536171;
            }
            #dashboardCard, #recentProjectsPanel {
                background: #ffffff;
                border: 1px solid #d7dde5;
                border-radius: 10px;
            }
    ...
    def _show_placeholder(self, feature_name: str) -> None:
        ...
        QMessageBox.information(
            ...
            (
                ...
                "Milestone 1 provides the polished GUI shell only."
            ),
        )
    ...
    def _show_about_dialog(self) -> None:
        ...
        QMessageBox.about(
            ...
            (
                "pyLOCO GUI\n\n"
                "Milestone 1 polished application shell built with PySide6.\n"
