...
bef = read_str_file('/machfs/MDT/2026/2026_01_18/strengthscomparison/AfterFILO_13_12_2020.ts')

#bef = read_str_file('/machfs/MDT/2026/2026_01_18/strengthscomparison/operation_before_realignment.ts')
...
for c, k in enumerate(['quad', 'sext', 'oct', 'hst', 'vst', 'sqp']):
    b = bef[k]-des[k]
    a = aft[k]-des[k]
    ax[c].plot(b, label=f'before (Dec 2020), std= {np.std(b):2.2e}')
    ax[c].plot(a, label=f'after  (Jan 2026), std= {np.std(a):2.2e}')
    ax[c].set_ylabel(k)
